    ...
    evaluate(){
        for(let p of this.population){
            ...
            for(let s of this.species){
                let d = Genome.compatibilityDistance(p.genome, s.mascot.genome, this.c1,this.c2,this.c3, this.N);
                // console.log(d);
                if(d < this.DT){
                    s.members.push(p);
        ...
        for(let p of this.population){
            ...
            if(Math.random() < this.ADD_NODE_RATE){
                p.genome.addNodeMutation(CONNECTIONINNOVATION, NODEINNOVATION);
            }
